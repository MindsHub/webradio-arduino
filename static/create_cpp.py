...
VARIABLE_REGEX = r"\s*(.*(?:\bextern\b).*)__attribute__\s*\(\s*\(\s*static_file\s*\(\s*\"([^\)]+)\"\s*\)\s*\)\s*\)"
...
with open("include/" + HPP_FILE) as f:
    for line in f:
        ...
        if m:
            variables.append((m.group(1).replace("extern", "").replace("PROGMEM", "").strip(), m.group(2)))
...
for varname, filename in variables:
    ...
    res += f"{varname} PROGMEM = R\"{RAW_STRING_WRAPPER}({content}){RAW_STRING_WRAPPER}\";\n"
